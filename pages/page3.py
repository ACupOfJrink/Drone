...
st.header("目标问题")
st.markdown('''假设队形变换在无障碍物情况下进行变换，则无人机 从 到 的所有路线中直线路径最短，那么问题就可以将队形变化变成优化问题。该优化问题需要求解在最小时间内的队形变化方案，该优化问题实际上是一个指派问题。若编队中有 架飞机，则潜在的指派方案就有 种。目标函数为能量耗费 和使用时间 的加权平均，则优化问题的数学表达式为''')
st.markdown(r'''$$min\;\;J()=\alhpa E(k)+(1-\alpha) \textit{T}(k) \\ \begin{cases} \textit{E}(k) = \sum^{n}_{i=1} \textit{E_i}(k) \\ \textit{T}(k)=max\{ t_1(k),t_2(k), \codts , t_n(k) , \alpha\in [0,1]\} \end{cases}}$$  ''')
